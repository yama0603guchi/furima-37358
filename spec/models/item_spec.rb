...
RSpec.describe Item, type: :model do
  before do
    @item = FactoryBot.build(:iten)
  end

  describe '商品出品機能' do
    context '商品の出品がうまくいく時' do
      it '全ての値が入力されていれば出品できる'
    ...
    context '商品の出品がうまくいかない時' do
      ...
      it 'item_condition_idが未選択だと出品できない' do
        ...
        @item.valid?
        expect(@item.errors.full_messages).to include "Item condition id  can't be blank"
      end
      it 'postage_player_idが未選択だと出品できない' do
        @item.postage_player_id = 1
        @item.valid?
        expect(@item.errors.full_messages).to include "Postage player id can't be blank"
      end
      it 'postage_area_idが未選択だと出品できない' do
        @item.postage_area_id = 1
        @item.valid?
        expect(@item.errors.full_messages).to include "Postage area id can't be blank"
      end
      it 'preparation_day_idが未選択だと出品できない' do
        @item.preparation_day_id = 1
        @item.valid?
        expect(@item.errors.full_messages).to include "Preparation day id can't be blank"
      end
      ...
      it '販売価格が¥9999999より多い時は出品できない' do
        ...
        @item.valid?
        expect(@item.errors.full_messages).to include('Price must be less than or to 9999999')
      end
